...
// 1.连接数据库
$link = mysqli_connect('localhost','liyang','ly1234..','haier');
if (!$link) {
    echo '{"err":0,"msg":"连接失败"}';
    die();
}
...
// 3.判断是登录还是注册
if ($type === 'login') {
   $login_sql = "select * from users where user='$user' and password='$pass'";
...
if ($type === 'add') {
    ...
    $insert_sql = "insert into users(user,password,email,phone) values('$user','$pass','$email','$phone')";
